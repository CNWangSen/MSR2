...
from model.MSR_agent_IPPO_Transformer import ActorUAVob,ActorUAVrt,ActorUSVrs,CriticUAVob,CriticUAVrt,CriticUSVrs
import torch
import numpy
import os
import random
...
class MSR_IPPO:
	def __init__(self,NUM_EPOCH=300000,RUN_ID=101,device="cuda:0",
			  collecting_paper_data=False,
			  parameter_sharing=True,using_advantage_norm=True,
			  using_state_norm=False,using_entropy_loss=True,
			  using_learning_rate_decay=True,using_gradient_clip=True,
			  rt_num=2,ob_num=3,rs_num=1,tg_num=80,
			  stochastic_action=True,map_scaling=1,current_amp=1):
		self.NUM_EPOCH=NUM_EPOCH
		self.RUN_ID=RUN_ID
		self.device=device
		self.alg_name="IPPO_Transformer"
		...
		self.wp=MyWrapper(use_group_reward=False,alg_name=self.alg_name,
		...
		if(self.parameter_sharing):
			...
			critic_cls=[CriticUAVob().to(self.device),CriticUAVrt().to(self.device),CriticUSVrs().to(self.device)]
			for p in self.wp.env.players:
				self.actors.append(actor_cls[p.cls])
				self.critics.append(critic_cls[p.cls])
		else:
			actor_cls=[ActorUAVob,ActorUAVrt,ActorUSVrs]
			critic_cls=[CriticUAVob,CriticUAVrt,CriticUSVrs]
			for p in self.wp.env.players:
				self.actors.append(actor_cls[p.cls]().to(self.device))
				self.critics.append(critic_cls[p.cls]().to(self.device))

		if(self.using_state_norm):
			self.state_norm_buffer_by_agent=[]
			for p in self.wp.env.players:
				self.state_norm_buffer_by_agent.append(RunningMeanStd(dim_obs=p.dim_s_obs))
	...
	def save(self):
		if(self.do_not_save):
			return
		print("-IPPO-SAVE",time.ctime())
		try:
			os.mkdir('pth/'+self.alg_name+"/")
		except:
			pass
		for i in range(len(self.actors)):
			index = 0 if self.parameter_sharing else i
			self.actors[i].save_checkpoint('pth/'+self.alg_name+'/'+str(self.RUN_ID)+"_"+self.alg_name,index)
		for i in range(len(self.critics)):
			index = 0 if self.parameter_sharing else i
			self.critics[i].save_checkpoint('pth/'+self.alg_name+'/'+str(self.RUN_ID)+"_"+self.alg_name,index)
		print("SAVE!")

	def load(self):
		for i in range(len(self.actors)):
			index = 0 if self.parameter_sharing else i
			self.actors[i].load_checkpoint('pth/'+self.alg_name+'/'+str(self.RUN_ID)+"_"+self.alg_name,index,self.device)
		for i in range(len(self.critics)):
			index = 0 if self.parameter_sharing else i
			self.critics[i].load_checkpoint('pth/'+self.alg_name+'/'+str(self.RUN_ID)+"_"+self.alg_name,index,self.device)
		print("LOAD!")
	...
	def get_J_action(self,J_obs,eval=False):
		...
		for i in range(len(J_obs)):
			...
			J_action.append(
				self.actors[i].get_s_action(tensor_obs.reshape(1,len_seq,len_feature),eval=(not self.stochastic_action))
			)
		return J_action
	...
	def get_traj(self,show=False,eval=False,reset_pos=True):
		...
		if(not eval):
			...
			J_obses_tensor=[torch.zeros(self.wp.env.max_traj_length,self.wp.env.players[i].dim_s_obs).to(self.device) for i in range(self.wp.env.N)]
			J_states_tensor=[]
			J_actions_tensor=[torch.zeros(self.wp.env.max_traj_length,self.wp.env.players[i].dim_s_act).to(self.device) for i in range(self.wp.env.N)]
			J_rewards_tensor=[torch.zeros(self.wp.env.max_traj_length,1).to(self.device) for i in range(self.wp.env.N)]
			J_next_obses_tensor=[torch.zeros(self.wp.env.max_traj_length,self.wp.env.players[i].dim_s_obs).to(self.device) for i in range(self.wp.env.N)]
			J_next_states_tensor=[]
			J_dones_tensor=[torch.zeros(self.wp.env.max_traj_length,1).to(self.device) for i in range(self.wp.env.N)]

		J_obs=self.wp.reset(reset_pos)
		if(eval==False):
		...
		for traj_i in range(self.wp.env.max_traj_length):
			...
			if(not eval):
				for actor_i in range(len(self.wp.env.players)):
					#traj_length,dim_obs
					J_obses_tensor[actor_i][traj_i]=torch.FloatTensor(J_obs[actor_i])
					#J_states_tensor[actor_i][traj_i]=
					J_actions_tensor[actor_i][traj_i]=torch.FloatTensor(J_action[actor_i])

					J_rewards_tensor[actor_i][traj_i]=J_reward[actor_i]
					J_next_obses_tensor[actor_i][traj_i]=torch.FloatTensor(J_next_obs[actor_i])
					#J_next_states_tensor[actor_i][traj_i]=
					J_dones_tensor[actor_i][traj_i]=1 if J_done[actor_i] else 0
					#state+=(J_obs[i]+J_action[i])
					#next_state+=(J_next_obs[i]+J_next_action[i])
			else:
				pass
			...
			J_obs=J_next_obs
			reward_0+=J_reward[2]+J_reward[3]+J_reward[4]
			reward_ep+=sum(J_reward)
			if(show):
				self.wp.show()
			if(J_done[0]):
				break
		...
		for ep in range(train_start,train_end):
			...
			for i in range(len(self.actors)):
				...
				if(self.using_learning_rate_decay):
					for p in self.actors[i].optimizer.param_groups:
						p['lr'] = 1e-3*(1-self.epoch/(self.NUM_EPOCH+1))
					for p in self.critics[i].optimizer.param_groups:
						p['lr'] = 1e-3*(1-self.epoch/(self.NUM_EPOCH+1))

				values=self.critics[i](J_obses[i])#5
				targets=self.critics[i](J_next_obses[i]).detach()#3
				targets*=0.98
				targets*=(1-J_dones[i])
				targets+=J_rewards[i]#1
				advantages=self.get_advantages(targets-values)#5-4
				
				alpha,beta=self.actors[i](J_obses[i])
				...
				for _ in range(10):
					...
					values=self.critics[i](J_obses[i])
