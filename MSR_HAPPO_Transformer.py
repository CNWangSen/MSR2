...
class MSR_HAPPO:
	def __init__(self,NUM_EPOCH=300000,RUN_ID=108,device ="cuda:1",collecting_paper_data=False,
			  ...
			  stochastic_action=True,map_scaling=1,current_amp=3):
		self.NUM_EPOCH=NUM_EPOCH
		self.RUN_ID=RUN_ID
		self.device=device

		self.alg_name="HAPPO_Transformer"
	...
	def load(self):
		...
		self.critics[0].load_checkpoint('pth/'+self.alg_name+'/'+str(self.RUN_ID)+"_"+self.alg_name,0,self.device)	
		print("LOAD!")
	...
	def train(self,save_train=True,train_num=0):
		...
		for ep in range(train_start,train_end):
			...
			if(self.using_learning_rate_decay):
				for p in self.critics[0].optimizer.param_groups:
					p['lr'] = 1e-3*(1-self.epoch/(self.NUM_EPOCH+1))

			values=self.critics[0](states[0]).detach()
			targets=self.critics[0](next_states[0]).detach()
			targets*=0.98
			targets*=(1-J_dones[0])
			targets+=J_rewards[0]
			...
			values=self.critics[0](states[0])
			critic_loss=loss_fn(values,targets)
			if(self.collecting_paper_data):
				for i in range(len(self.actors)):
					self.paper_data_J_critic_loss_train[i].append(critic_loss.detach().cpu().item())
	 
			self.critics[0].optimizer.zero_grad()
			critic_loss.backward()
			if(self.using_gradient_clip):
				torch.nn.utils.clip_grad_norm_(self.critics[0].parameters(), 0.5) 
			self.critics[0].optimizer.step()
...
if __name__ == '__main__':
	...
	if IS_TRAINING:
		...
		score = 0.0
		EVAL_RUN_NUM=1
		for i in range(EVAL_RUN_NUM):
			t.get_traj(show=True,eval=True)
			score+=t.wp.env.Final_Percent
		print(score/EVAL_RUN_NUM)
