...
class CriticUAVob(torch.nn.Module):
	def __init__(self):
		...
		self.fc_score=torch.nn.Sequential(
			orthogonal_init(torch.nn.Linear(dv_TG+dv_RS,128)),
			torch.nn.Tanh(),
			orthogonal_init(torch.nn.Linear(128,128)),
			torch.nn.Tanh(),
			orthogonal_init(torch.nn.Linear(128,1),gain=0.01),
		)
		self.optimizer = torch.optim.Adam(self.parameters(),lr=1e-4,eps=1e-5)
	...
	def load_checkpoint(self,checkpoint_file_prefix,index,device):
		self.load_state_dict(torch.load(checkpoint_file_prefix+"_critic_ob_"+str(index).zfill(2)+".pth",map_location=device))
...
class CriticUAVrt(torch.nn.Module):
	...
	def load_checkpoint(self,checkpoint_file_prefix,index,device):
		self.load_state_dict(torch.load(checkpoint_file_prefix+"_critic_rt_"+str(index).zfill(2)+".pth",map_location=device))
...
class CriticUSVrs(torch.nn.Module):
	...
	def load_checkpoint(self,checkpoint_file_prefix,index,device):
		self.load_state_dict(torch.load(checkpoint_file_prefix+"_critic_rs_"+str(index).zfill(2)+".pth",map_location=device))
