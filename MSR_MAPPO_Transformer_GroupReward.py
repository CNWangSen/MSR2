...
class MSR_MAPPO_GroupReward:
	def __init__(self,NUM_EPOCH=300000,RUN_ID=101,device ="cuda:2",collecting_paper_data=False,
			  parameter_sharing=True,using_advantage_norm=True,using_state_norm=False,
		...
		self.alg_name="MAPPO_Transformer_GroupReward"
		...
		if(self.parameter_sharing):
			...
			for p in self.wp.env.players:
				...
				self.critics.append(critic_cls[0]().to(self.device))
	...
	def train(self,save_train=True,train_num=0):
		...
		for ep in range(train_start,train_end):
			...
			for i in range(len(self.actors)):
				...
				values=self.critics[i](states[0])
				targets=self.critics[i](next_states[0]).detach()
				targets*=0.98
				targets*=(1-J_dones[i])
				targets+=J_rewards[i]
